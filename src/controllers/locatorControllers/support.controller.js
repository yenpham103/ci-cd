...
module.exports = {
    getSupportContent: async (ctx) => {
        ...
        const { name } = userData;
        try {
            const supportContent = await LocatorService.findByDomain(domain);
            ctx.body = {
                status: 'success',
                message: supportContent?.content || '',
            };
            logger.info(__filename, 'Get locator support content successfully', name, domain);
        } catch (error) {
    ...
    updateSupportContent: async (ctx) => {
        ...
        const { name } = userData;
        try {
            await LocatorService.updateContent(domain, content, name);
            ctx.body = {
                status: 'success',
            };
            logger.info(__filename, 'Update locator support content successfully', name, domain);
        } catch (error) {
            logger.error(__filename, error.message, name, domain);
    ...
    getVersionList: async (ctx) => {
        ...
        const { name } = userData;
        try {
            const versions = await LocatorService.getVersionList(domain, parseInt(limit));
            ctx.body = {
                status: 'success',
                message: versions,
            };
            logger.info(__filename, 'Get locator support version list successfully', name, domain);
        } catch (error) {
            logger.error(__filename, error.message, name, domain);
            ctx.throw(error.status, error.message);
        }
    ...
    getVersionContent: async (ctx) => {
        ...
        const { name } = userData;
        try {
            const version = await LocatorService.getVersion(id, domain);
            ctx.body = {
                status: 'success',
                message: version,
            };
